2025-03-24
On going: 将采购订单，销售订单和库存管理级联

2025-02-28

1.调整markup和excise的税率
2.开始增加财务管理功能
3.优化产品名称大小写会被认成两个名称的问题

2025-02-21
ADD：
销售订单管理中，显示区域，产品编号和采购订单号中间插入一列，名称为SKU CLS。
可以通过鼠标选择单元格进行复制粘贴
...
On going:
    ...
    46. 订单删除，撤销删除功能优化
    46.1 将采购订单管理中的删除订单功能移植到销售订单管理和产品管理中。销售订单管理的删除订单功能通过销售订单号来实现，每次只能删除一个订单号的订单。 产品管理的删除产品功能也是类似，通过填写SKU CLS号来删除对应产品信息，每次只能删除一个产品。
    46.1.1 优化后的功能在采购订单管理，销售订单管理和产品管理中的名称分别是：删除采购订单，删除销售订单，删除产品。当成功删除用户选择的订单/产品后，将自动保存被删除的订单/产品信息至excel文件。文件名分别为：删除采购订单记录，删除销售订单记录，删除产品记录。用于撤销删除功能。即，撤销删除功能需要的信息都应该被存储下来。
    46.1.2 此记录的前三列为：删除时间；采购订单号/销售订单号/SKU CLS；ITEM Name（产品名称）
    46.2 撤销删除功能，更改为：步骤：1.用户点击撤销删除按键 2.弹出窗口，列表形式显示历史中所有被删除的订单/产品 3.用户选择某一个被删除的订单/产品 4.用户点击恢复 5.系统提问是否确认恢复此订单/产品 6.用户点击确认 7.此订单/产品被恢复并被高亮显示。
    46.2.1 针对：弹出窗口，列表形式显示历史中所有被删除的订单/产品。此需求中，列表的第一列为删除的时间；第二列为订单号（采购订单号、销售订单号）/SKU CLS； 第三列为订单/产品对应的ITEM Name（产品名称）
    46.2.2 针对：弹出窗口，列表形式显示历史中所有被删除的订单/产品。此需求中，列表显示的内容来源于对应的删除采购订单记录\删除销售订单记录\删除产品记录。
    46.2.3 针对：用户选择某一个被删除的订单/产品。此需求，要求用户最多只能选择一个，不支持多选功能。
    46.2.4 若操作不成功，则弹窗提示失败原因，比如订单号、产品名称冲突等。
    46.2.4.1 此逻辑建立在，程序应在操作前先检查是否存在操作失败的可能，若存在则直接提示。尽量不要更改了数据库数据后才出现错误提示。防止数据被破坏。此条需求chat gpt请给我详细分析。


    47.销售订单管理中，输入框的产品编号现在是下拉菜单的形式。我想将下拉菜单仍然保留，增加用户打字输入筛选的功能。筛选方式如工具中存在的过滤条件功能，支持*来模糊过滤。当用户输入的内容存在，那么下拉菜单就会显示所有相关的产品。
    48.销售订单管理中，将采购订单管理中的导出订单功能移植过来。操作逻辑和操作方法相同。

    49. 采购订单中，显示区域的wholsale price， invoice price 改为支持手动输入。
待实现：
    2024-11-04
    整理架构：
